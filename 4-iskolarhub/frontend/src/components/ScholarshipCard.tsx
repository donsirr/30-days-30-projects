        ...
        >
            ...
            <div className="p-6">
                ...
                <div className="flex justify-between items-start mb-4">
                    <div className="flex gap-4">
                        ...
                        <div className="relative size-14 shrink-0 flex items-center justify-center">
                            <svg className="size-full -rotate-90">
                                <circle cx="28" cy="28" r="22" className="stroke-slate-100" strokeWidth="4" fill="none" />
                                <motion.circle
                                    ...
                                    className={clsx(
                                        "stroke-current",
                                        matchPercentage >= 90 ? "text-green-600" : matchPercentage >= 75 ? "text-primary" : "text-orange-500"
                                    )}
                                    strokeWidth="4"
                                    strokeLinecap="round"
                                    fill="none"
                            ...
                            <span className="absolute text-xs font-bold text-slate-700">{matchPercentage}%</span>
                        </div>

                        <div>
                            <h3 className="font-bold text-lg text-slate-900 leading-tight mb-1 group-hover:text-primary transition-colors cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                                {title}
                            </h3>
                    ...
                    <button className="text-slate-300 hover:text-primary transition-colors p-2 hover:bg-blue-50 rounded-full active:scale-90">
                        <Bookmark size={20} />
                    </button>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2 mb-4">
                    ...
                    {amount && (
                        <div className="px-3 py-1.5 rounded-lg bg-blue-50 text-blue-700 border border-blue-100 text-xs font-bold">
                            {amount}
                        </div>
                    )}
                </div>
                ...
                <button
                    ...
                    className="w-full flex items-center justify-between text-sm font-semibold text-slate-400 hover:text-primary transition-colors py-2 border-t border-slate-50 mt-2"
                >
                ...
                {isExpanded && (
                    ...
                    >
                        <div className="p-6 pt-2 space-y-3">
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Eligibility Logic</p>
                                ...
                                <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500">{req.value}</p>
                                        </div>
                                    </div>
                                    {req.isMet ? (
                            ...
                            <button className="w-full mt-4 bg-primary text-white py-3 rounded-xl font-bold text-sm shadow-md hover:bg-blue-800 active:scale-95 transition-all flex items-center justify-center gap-2">
                                <span>Apply Now</span>
                                <ArrowRight size={16} />
                            </button>
                        </div>
